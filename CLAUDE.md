...
1. **제목 형식**: `type: 간결한 설명` (50자 이내)
2. **본문**: 변경 사항을 bullet point로 명확히 기술
3. **Claude 서명 금지**: 커밋 메시지에 Claude 관련 서명이나 표시 절대 금지
4. **한글/영문**: 프로젝트 커밋 히스토리와 일관성 유지
...
### 좋은 커밋 메시지 예시
```
chore: 모든 의존성 최신 버전 업데이트 및 Express 5 마이그레이션

- React 19, Vite 7, Electron 37 등 주요 라이브러리 최신 버전 업데이트
- Express 4에서 Express 5로 업그레이드
- TypeScript 타입 오류 수정
- ESM 모듈 호환성 문제 해결
```
