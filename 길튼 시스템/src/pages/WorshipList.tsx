...
export default function WorshipList() {
  const { worships, worshipTypes, deleteWorship } = useWorshipStore();
  ...
  const currentRole = currentProfile ? getRoleById(currentProfile.roleId) : undefined;
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          {currentProfile && (
            <div className="flex items-center gap-3 px-5 py-3 bg-white rounded-xl shadow-md border-2 border-slate-200">
              <div className={`w-10 h-10 ${currentProfile.color} rounded-full flex items-center justify-center text-xl`}>
                {currentRole?.icon}
              </div>
              <div>
              ...
              {worshipTypes.map((type) => {
                ...
                return (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl font-semibold transition-all ${
                      isSelected
                        ? `bg-gradient-to-r ${colorOption.gradient} text-white shadow-md`
                        : `${colorOption.badge} hover:shadow-md`
                    }`}
                  >
                    {type.name}
                  </button>
                    ...
                    <div className="mb-3">
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        ...
                        <div className="text-sm text-slate-500">
                          악보 {worship.sheets.length}개
                        </div>
                        {worshipType && (
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getColorOption(worshipType.color).badge}`}>
                            {worshipType.name}
                          </span>
                        )}
                      </div>
