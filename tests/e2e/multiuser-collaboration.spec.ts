...
// 브라우저별 사용자 세션 생성
async function createUserSession(context: BrowserContext, user: TestUser): Promise<Page> {
  ...
  // 예배 목록에서 테스트용 예배 선택
  await page.waitForSelector('[data-testid="worship-list"]', {
    timeout: 10000,
  });
...
// 그리기 동작 시뮬레이션
async function simulateDrawing(page: Page, startX: number, startY: number, endX: number, endY: number): Promise<void> {
