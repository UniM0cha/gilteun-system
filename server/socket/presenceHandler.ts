...
export function setupPresenceHandler(io: Server, socket: Socket): void {
  // 예배 입장
  socket.on("join:worship", (data: { worshipId: string; profileId: string }) => {
    ...
    try {
      ...
      if (profile) {
        name = profile.name;
        const roleData = db.select().from(roles).where(eq(roles.id, profile.roleId)).get();
        if (roleData) {
          role = roleData.name;
  ...
  // 현재 보고 있는 페이지 변경
  socket.on("page:change", (data: { worshipId: string; sheetId: string }) => {
    const worship = presenceMap.get(data.worshipId);
    if (worship) {
  ...
  // 페이지 호출 (스포트라이트)
  socket.on("page:spotlight", (data: { worshipId: string; sheetId: string; sheetTitle: string; profileId: string }) => {
    const worship = presenceMap.get(data.worshipId);
    let senderName = "익명";
    let senderRole = "";
    if (worship) {
