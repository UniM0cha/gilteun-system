...
export function setupCommandHandler(io: Server, socket: Socket): void {
  socket.on("command:send", (data: { worshipId: string; commandId: string; profileId: string }) => {
    try {
      const command = db.select().from(commands).where(eq(commands.id, data.commandId)).get();
      if (!command) return;
