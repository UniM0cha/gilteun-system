...
export const Worship = () => {
    ...
    const totalPages = 3;
    // 드로잉 hook 사용
    const { drawingEvents, isDrawingMode, toolSettings, viewport, sendDrawingEvent, toggleDrawingMode, updateToolSettings, updateViewport, } = useDrawing({
    ...
    // 명령 hook 사용
    const { commands, commandTemplates, sendCommand, expireCommand, addCommandTemplate, updateCommandTemplate, deleteCommandTemplate, } = useCommand({
        userId: currentUser?.id || '',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            <ScoreNavigation currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} viewport={viewport} onViewportChange={updateViewport}/>

            <DrawingToolbar toolSettings={toolSettings} onToolSettingsChange={updateToolSettings} isDrawingMode={isDrawingMode} onDrawingModeChange={toggleDrawingMode}/>

            {/* 명령 시스템 */}
            {isManagingTemplates ? (<CommandTemplateManager templates={commandTemplates} onAdd={addCommandTemplate} onUpdate={updateCommandTemplate} onDelete={deleteCommandTemplate} onReorder={(templateIds) => {
                // 템플릿 순서 변경 구현
            ...
            }}/>) : (<CommandPanel userRole={currentUser?.role || 'session'} onSendCommand={sendCommand} commandTemplates={commandTemplates} onManageTemplates={() => setIsManagingTemplates(true)}/>)}

            {isManagingTemplates && (<Button variant="outline" onClick={() => setIsManagingTemplates(false)} className="w-full">
                명령 패널로 돌아가기
              </Button>)}
          </div>
          ...
          <div className="lg:col-span-3">
            <div className="space-y-4">
              ...
              <div className="relative">
                <ScoreViewer score={currentScore || undefined} currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} viewport={viewport} onViewportChange={updateViewport} drawingEvents={drawingEvents} isDrawingMode={isDrawingMode} toolSettings={toolSettings} onDrawingEvent={sendDrawingEvent} userId={currentUser?.id || ''}/>
