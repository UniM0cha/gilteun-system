...
export const useCommand = ({ userId, userName, userInstrument, isConnected }) => {
    const [commands, setCommands] = useState([]);
    const [commandTemplates, setCommandTemplates] = useState([]);
    const socketService = getSocketService();
    // 명령 수신 처리
    const handleCommandReceived = useCallback((command) => {
        setCommands((prev) => {
            // 중복 명령 방지
    ...
    // 명령 만료 처리
    const expireCommand = useCallback((commandId) => {
    ...
    // 명령 템플릿 저장
    const saveCommandTemplates = useCallback((templates) => {
        try {
    ...
    // 새 템플릿 추가
    const addCommandTemplate = useCallback((template) => {
        const newTemplate = {
            ...template,
    ...
    // 템플릿 수정
    const updateCommandTemplate = useCallback((templateId, updates) => {
        const updated = commandTemplates.map((template) => template.id === templateId ? { ...template, ...updates, updatedAt: new Date() } : template);
        saveCommandTemplates(updated.sort((a, b) => a.order - b.order));
    }, [commandTemplates, saveCommandTemplates]);
    // 템플릿 삭제
    const deleteCommandTemplate = useCallback((templateId) => {
        const updated = commandTemplates.filter((template) => template.id !== templateId);
        saveCommandTemplates(updated);
    }, [commandTemplates, saveCommandTemplates]);
    // 템플릿 순서 변경
    const reorderCommandTemplates = useCallback((templateIds) => {
    ...
    useEffect(() => {
        const cleanup = setInterval(() => {
            const now = Date.now();
            setCommands((prev) => prev.filter((command) => {
                const commandTime = new Date(command.timestamp).getTime();
