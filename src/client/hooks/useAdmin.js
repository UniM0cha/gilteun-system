...
export const useAdmin = () => {
    ...
    // 시스템 로그 조회
    const fetchSystemLogs = useCallback(async (limit = 100, level) => {
        try {
    ...
    // 사용자 강제 퇴장
    const disconnectUser = useCallback(async (socketId, reason) => {
        try {
            setIsLoading(true);
    ...
    // 악보 업로드
    const uploadScore = useCallback(async (file, title, worshipId) => {
        try {
            setIsLoading(true);
