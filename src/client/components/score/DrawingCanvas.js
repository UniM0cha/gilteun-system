...
export const DrawingCanvas = ({ width, height, viewport, toolSettings, isDrawingMode, onDrawingEvent, drawingEvents, currentPage, scoreId, userId, }) => {
    ...
    // 드로잉 중
    const draw = (e) => {
        if (!isDrawing || !isDrawingMode || !currentEventId)
            return;
        e.preventDefault();
        const point = 'touches' in e
            ? getCanvasCoordinates(e.touches[0]?.clientX || 0, e.touches[0]?.clientY || 0)
    ...
    return (<canvas ref={canvasRef} className="absolute inset-0 touch-none" style={{
