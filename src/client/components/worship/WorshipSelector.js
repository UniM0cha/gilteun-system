...
export const WorshipSelector = ({ onSelect }) => {
    ...
    return (<Card className="max-w-md mx-auto">
      ...
      <CardContent className="space-y-6">
        <div className="space-y-3">
          {worships.length === 0 ? (<p className="text-muted-foreground text-center">현재 진행 중인 예배가 없습니다.</p>) : (worships.map((worship) => (<Button key={worship.id} variant={selectedWorshipId === worship.id ? 'default' : 'outline'} className="w-full p-4 h-auto justify-start" onClick={() => setSelectedWorshipId(worship.id)}>
        ...
        {selectedWorshipId && (<Button onClick={handleSelect} className="w-full">
            예배 참여하기
          </Button>)}
      </CardContent>
    </Card>);
