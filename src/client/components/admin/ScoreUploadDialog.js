...
export const ScoreUploadDialog = ({ open, onOpenChange, onSuccess }) => {
      ...
      <DialogContent className="max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label>악보 이미지</Label>
            <Card className={`border-2 border-dashed transition-colors ${dragOver ? 'border-primary bg-primary/5' : 'border-muted-foreground/25 hover:border-muted-foreground/50'}`} onDragOver={(e) => {
            e.preventDefault();
            setDragOver(true);
        }} onDragLeave={() => setDragOver(false)} onDrop={handleDrop}>
              <CardContent className="p-6">
                {selectedFile && previewUrl ? (<div className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="예: 놀라운 은혜" required/>
          </div>

          {/* 예배 선택 */}
          <div className="space-y-2">
            <Label htmlFor="worship">예배 선택</Label>
            <select id="worship" value={selectedWorshipId} onChange={(e) => setSelectedWorshipId(e.target.value)} className="w-full px-3 py-2 border border-input rounded-md bg-background text-foreground" required>
