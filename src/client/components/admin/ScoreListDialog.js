...
export const ScoreListDialog = ({ open, onOpenChange }) => {
    ...
    const getWorshipName = (worshipId) => {
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-hidden flex flex-col space-y-4">
          ...
          <div className="flex items-center space-x-4">
            ...
            <select value={selectedWorshipId} onChange={(e) => setSelectedWorshipId(e.target.value)} className="px-3 py-2 border border-input rounded-md bg-background text-foreground">
              <option value="">전체 악보</option>
              {worships.map((worship) => (<option key={worship.id} value={worship.id}>
          ...
          <div className="flex-1 overflow-y-auto space-y-3">
            {filteredScores.length === 0 ? (<Card>
              ...
              </Card>) : (filteredScores.map((score) => (<Card key={score.id} className="cursor-pointer hover:bg-accent/50 transition-colors">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        {/* 악보 미리보기 */}
                        <div className="w-16 h-16 bg-muted rounded border flex items-center justify-center">
                          {score.filePath ? (<img src={score.filePath} alt={score.title} className="w-full h-full object-cover rounded" onError={(e) => {
                    // 이미지 로드 실패 시 아이콘 표시
                    ...
                    target.parentElement.innerHTML =
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center space-x-2 mt-1">
                            ...
                            {score.createdAt && (<span className="text-xs text-muted-foreground">
                      ...
                      <div className="flex items-center space-x-2">
                        {score.filePath && (<Button variant="outline" size="sm" onClick={() => window.open(score.filePath, '_blank')}>
                            <Eye className="h-4 w-4"/>
                          </Button>)}

                        <Button variant={deleteConfirm === score.id ? 'destructive' : 'outline'} size="sm" onClick={() => handleDelete(score.id)} disabled={isLoading}>
                          <Trash2 className="h-4 w-4"/>
                          {deleteConfirm === score.id ? '확인' : ''}
                        </Button>

                        {deleteConfirm === score.id && (<Button variant="outline" size="sm" onClick={() => setDeleteConfirm(null)}>
                            취소
                          </Button>)}
                      </div>
                    </div>
