...
export const PerformanceMonitor: React.FC<PerformanceMonitorProps> = memo(
  ({ metrics, visible = true, compact = false, position = 'top-right' }) => {
    ...
    if (compact) {
      return (
        <div className={`absolute ${getPositionClasses()} z-40`}>
          <div className="bg-opacity-80 rounded-lg bg-black px-3 py-2 text-sm text-white">
    ...
    return (
      <div className={`absolute ${getPositionClasses()} z-40`}>
        <div className="bg-opacity-80 rounded-lg bg-black px-4 py-3 text-sm text-white shadow-lg backdrop-blur-sm">
          <div className="space-y-2">
            <div className="text-center font-semibold">성능 모니터</div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className={getLatencyColor(metrics.inputLatency)}>{metrics.inputLatency.toFixed(1)}ms</span>
            </div>
            ...
            {/* 상태 표시기 */}
            <div className="mt-2 border-t border-gray-600 pt-2 text-center text-xs">
              {metrics.performanceScore >= 80 && <span className="text-green-400">✓ 우수한 성능</span>}
              {metrics.performanceScore >= 60 && metrics.performanceScore < 80 && (
                <span className="text-yellow-400">⚠ 보통 성능</span>
              )}
              {metrics.performanceScore < 60 && <span className="text-red-400">⚠ 성능 개선 필요</span>}
            </div>

            {/* 성능 개선 제안 */}
            {metrics.performanceScore < 70 && (
              <div className="mt-1 text-xs text-gray-300">
                {metrics.inputLatency > 16 && <div>• Apple Pencil 지연시간 높음</div>}
                {metrics.fps < 45 && <div>• 프레임률 저하</div>}
                {metrics.memoryUsage > 500 && <div>• 메모리 사용량 높음</div>}
              </div>
            )}
          </div>
        </div>
