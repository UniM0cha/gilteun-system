...
export function setupSocketHandlers(io) {
    io.on('connection', (socket) => {
        ...
        socket.on('user:join', async (data) => {
            ...
            // 다른 사용자들에게 새 사용자 알림
            socket.to(`worship:${data.worshipId}`).emit('users:update', Array.from(connectedUsers.values()).filter((user) => user.worshipId === data.worshipId));
        ...
        // 악보 페이지 변경
        socket.on('score:page-change', async (data) => {
        ...
        socket.on('command:send', (data) => {
            ...
            if (target === 'all') {
                ...
                const targetSockets = Array.from(connectedUsers.entries())
                    .filter(([, session]) => session.worshipId === userSession.worshipId && data.targetUserIds.includes(session.userId))
        ...
        socket.on('disconnect', () => {
            ...
            if (userSession) {
                ...
                // 다른 사용자들에게 사용자 퇴장 알림
                socket.to(`worship:${userSession.worshipId}`).emit('users:update', Array.from(connectedUsers.values()).filter((user) => user.worshipId === userSession.worshipId));
