...
router.post('/scores/upload', upload.single('scoreFile'), async (req, res) => {
    ...
    `);
        ...
        const orderIndex = countStmt?.get(worshipId)?.count || 0;
...
router.delete('/scores/:id', async (req, res) => {
    try {
        ...
        // 파일 삭제
        const filePath = path.join(process.cwd(), 'uploads', 'scores', path.basename(score.file_path));
...
router.get('/logs', (_req, res) => {
    try {
        ...
        const logs = adminService.getLogs(parseInt(limit) || 100, level);
        return res.json({
            success: true,
            data: logs,
        });
