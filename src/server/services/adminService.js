...
class AdminService {
    ...
    // 연결된 사용자 관리
    addConnectedUser(socketId, userSession) {
    ...
    // 로그 관리
    addLog(level, message, component, details) {
        const log = {
            id: this.generateId(),
            level,
            message,
    ...
    async getDatabaseStats() {
        ...
        try {
            const stats = {
                worships: db.prepare?.('SELECT COUNT(*) as count FROM worships')?.get?.()?.count ||
                    0,
                scores: db.prepare?.('SELECT COUNT(*) as count FROM scores')?.get?.()?.count || 0,
                drawings: db.prepare?.('SELECT COUNT(*) as count FROM score_drawings')?.get?.()
                    ?.count || 0,
                templates: db.prepare?.('SELECT COUNT(*) as count FROM command_templates')?.get?.()
                    ?.count || 0,
    ...
    async cleanupSystem() {
      ...
      `);
            ...
            this.addLog('info', `시스템 정리 완료: 드로잉 ${deletedDrawings.changes}개, 예배 ${deletedWorships.changes}개 삭제`, 'AdminService');
            return {
                success: true,
