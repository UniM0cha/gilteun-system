...
// 임시 목업 데이터
const mockUsers = [
  { id: 'user-1', name: '김찬양', role: 'leader', isOnline: true, lastSeen: new Date().toISOString() },
  {
    id: 'user-2',
    name: '이기타',
    role: 'guitarist',
...
router.get('/', (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Internal Server Error', message: '사용자 목록 조회 중 오류 발생' });
  }
});
...
router.patch('/:id/status', (req, res) => {
  try {
    ...
    if (userIndex === -1) {
      return res.status(404).json({ error: 'Not Found', message: `사용자 ${id}를 찾을 수 없습니다` });
    }
    ...
    res.status(500).json({ error: 'Internal Server Error', message: '사용자 상태 업데이트 중 오류 발생' });
  }
});
