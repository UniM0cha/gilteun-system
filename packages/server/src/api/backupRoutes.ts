    ...
    db.transaction(() => {
      ...
      if (data.data.worships) {
        ...
        for (const item of data.data.worships) {
          insertWorship.run(
            item.id, item.type_id, item.name, item.date,
      ...
      if (data.data.scores) {
        ...
        for (const item of data.data.scores) {
          insertScore.run(
            item.id, item.worship_id, item.title, item.file_path,
            item.order_index, item.created_at, item.updated_at
          );
        }
      }
      ...
      if (data.data.command_templates) {
        ...
        for (const item of data.data.command_templates) {
          insertTemplate.run(
            item.id, item.title, item.content, item.order_index,
            item.created_at, item.updated_at
          );
        }
      }
