...
export const useCommand = ({
  ...
  // 명령 템플릿 저장
  const saveCommandTemplates = useCallback((templates: CommandTemplate[]) => {
    try {
  ...
  // 새 템플릿 추가
  const addCommandTemplate = useCallback((template: Omit<CommandTemplate, 'id' | 'userId' | 'createdAt' | 'updatedAt'>) => {
  ...
  // 템플릿 수정
  const updateCommandTemplate = useCallback((templateId: string, updates: Partial<CommandTemplate>) => {
  ...
  // 템플릿 삭제
  const deleteCommandTemplate = useCallback((templateId: string) => {
    const updated = commandTemplates.filter(template => template.id !== templateId);
    saveCommandTemplates(updated);
  }, [commandTemplates, saveCommandTemplates]);

  // 템플릿 순서 변경
  const reorderCommandTemplates = useCallback((templateIds: string[]) => {
    const reordered = templateIds.map((id, index) => {
