...
describe('shadcn/ui 테마 적용 테스트', () => {
  ...
  it('Button variant들이 올바른 테마 클래스를 적용해야 함', () => {
    const { rerender } = render(<Button data-testid="test-button" variant="outline">Outline</Button>);
    let button = screen.getByTestId('test-button');
    expect(button).toHaveClass('border-input');
    expect(button).toHaveClass('bg-background');
    expect(button).toHaveClass('hover:bg-accent');

    rerender(<Button data-testid="test-button" variant="secondary">Secondary</Button>);
    button = screen.getByTestId('test-button');
    expect(button).toHaveClass('bg-secondary');
    expect(button).toHaveClass('text-secondary-foreground');

    rerender(<Button data-testid="test-button" variant="destructive">Destructive</Button>);
    button = screen.getByTestId('test-button');
  ...
  it('Badge 컴포넌트가 올바른 테마 클래스를 적용해야 함', () => {
    ...
    rerender(<Badge data-testid="theme-badge" variant="outline">아웃라인 뱃지</Badge>);
    badge = screen.getByTestId('theme-badge');
    expect(badge).toHaveClass('border');
    expect(badge).toHaveClass('text-foreground');

    rerender(<Badge data-testid="theme-badge" variant="destructive">경고 뱃지</Badge>);
    badge = screen.getByTestId('theme-badge');
  ...
  it('muted 색상 변수들이 올바르게 적용되어야 함', () => {
    render(
      <div>
        <p data-testid="muted-text" className="text-muted-foreground">흐린 텍스트</p>
        <div data-testid="muted-bg" className="bg-muted">흐린 배경</div>
      </div>
    );
  ...
  it('destructive 색상이 오류 상황에 올바르게 적용되어야 함', () => {
    render(
      <div>
        <p data-testid="error-text" className="text-destructive">오류 메시지</p>
        <Button data-testid="error-button" variant="destructive">삭제</Button>
      </div>
    );
